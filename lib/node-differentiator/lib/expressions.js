...
const additionToString = function() {
    ...
    if(leftArgExpressionType.arity === 2 && expressionType < leftArgExpressionType.precedence) {
        leftArgString = parenthesize(this.argLeft.toString());
    } else leftArgString = this.argLeft.toString();
    let rightArgString;
    if(rightArgExpressionType.arity === 2 && expressionType < rightArgExpressionType.precedence) {
...
const multiplicationToString = function() {
    ...
    const leftArgExpressionType = this.argLeft.expressionType;
    const rightArgExpressionType = this.argRight.expressionType;
    if(leftArgExpressionType.arity === 2 && expressionType.precedence < leftArgExpressionType.precedence) {
        leftArgString = parenthesize(this.argLeft.toString());
    } else leftArgString = this.argLeft.toString();
    let rightArgString;
...
const exponentialToString = function() {
    ...
    const expressionType = this.expressionType;
    let baseToString;
    if((baseExpressionType.arity === 2 && expressionType.precedence < baseExpressionType.precedence)
        || baseExpressionType === expressionTypes.basicFunction) {
...
const cotExpressionDerivative = function() {
    const cotDerivativeRule = 'd/dx cot(x) = -cosec^2(x)';
    const expressionDerivative = derivativeExpression(this); 
    if(this.isConstant()) {
        return constantDerivative.call(this);
    }
...
const cosecExpressionToString = function() {
    return 'cosec' + '(' + this.argument.toString() + ')'
};

const cosecExpressionDerivative = function() {
    const cosecDerivativeRule = 'd/dx cosec(x) = -cosec(x)*cot(x)';
    const expressionDerivative = derivativeExpression(this); 
    if(this.isConstant()) {
        return constantDerivative.call(this);
    }
    ...
    const argument = this.argument;
    const argumentDerivative = argument.derivative();
    const rules = [cosecDerivativeRule, chainRule];
    const appliedRule = multiplicationExpression(
...
const cosecExpression = (argument) => {
    const ret = {
        argument,
        expressionType: expressionTypes.basicFunctionExpression,
        toString: cosecExpressionToString,
        derivative: cosecExpressionDerivative,
        isConstant: unaryIsConstant,
    };
    return ret;
}

exports.cosecExpression = cosecExpression;
